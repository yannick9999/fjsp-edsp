...
# validation set must exist (created by generate_val.sh)
if [[ ! -f "val/seed${SEED}/validation_set.json" ]]; then
    echo "ERROR: missing val/seed${SEED}/validation_set.json (run generate_val first)" >&2
    exit 1
fi
