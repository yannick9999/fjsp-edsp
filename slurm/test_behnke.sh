#!/bin/bash
#SBATCH --job-name=fjsp_test_behnke
...
#SBATCH --array=0-2
#SBATCH --output=logs/test_behnke_seed%a_%j.out
#SBATCH --error=logs/test_behnke_seed%a_%j.err
...
# final policies must exist (Echeverria saves .pth, not .pt)
if ! ls "${MODEL_DIR}"/*.pth 1>/dev/null 2>&1; then
    echo "ERROR: No .pth file found in ${MODEL_DIR}" >&2
    exit 1
fi

echo "=== Testing Behnke seed=${SEED} exp=${EXP_NAME} model_dir=${MODEL_DIR} ==="
srun nvidia-smi || true

exec srun "${PYTHON}" test_behnke.py --seed "${SEED}"
